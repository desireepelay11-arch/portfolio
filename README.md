# DESIREÉ PELAY
### Strategic HR Business Partner (HRBP) | Organizational Design & Talent Management

Soy un HRBP enfocado en **traducir la visión del negocio en estrategias de personas, cultura y desarrollo de alto impacto**. Mi enfoque combina el diseño organizacional, la gestión del talento y las neurociences aplicadas al liderazgo para construir equipos eficientes en la era de la Inteligencia Artificial.

A diferencia del enfoque tradicional de Recursos Humanos, utilizo **modelado de datos y pruebas de concepto técnicas** para visibilizar riesgos culturales y diseñar intervenciones de upskilling/reskilling precisas.

---

## 🚀 Proyecto Técnico Insignia (Open Source Research)

### 🧠 [Simulador de Sesgos Cognitivos en Sistemas de IA Locales](./simulacion_sesgos_ia.ipynb)
Un modelo predictivo en Python que traduce las tesis de Jessica Nordell (*"El fin del sesgo"*) a métricas de negocio. 

* **El Problema de Negocio:** Cuando un HRBP diseña estrategias de adopción de IA local (como asistentes integrados en Workday, SuccessFactors o ERPs privados), entrenar estos modelos con datos históricos puede automatizar y escalar los sesgos de liderazgo pasados a nivel de software.
* **El Modelo:** Simula el impacto sistémico del sesgo de afinidad en la **Tasa de Innovación** corporativa y la **Retención de Talento**, demostrando cómo las intervenciones estructuradas y el rediseño organizacional estabilizan el ROI del capital humano.
* **Ejecución:** Podés correr el cuaderno interactivo haciendo clic en el botón azul de Google Colab dentro del archivo.

---

## 💼 Trayectoria Profesional & Casos de Éxito

* **[Acceder a mi CV Interactivo & Portafolio de Proyectos de HR](./index.html)** *(Nota: Reemplazá index.html por el nombre exacto de tu archivo HTML actual si se llama distinto)*

En mi experiencia interactiva vas a encontrar el desglose de los proyectos que lideré en las áreas de **Talent Management, Diseño Organizacional y Desarrollo de Liderazgo**, conectando los objetivos de la mesa directiva con ejecuciones culturales medibles.

---

## ✉️ Consultorías & Estrategia de Negocio

Colaboro con CoEs/HRBPs y equipos de innovación que buscan diseñar las organizaciones para el Futuro del Trabajo, poniendo en el centro el factor humano.

* **LinkedIn:** [Tu Perfil de LinkedIn](https://linkedin.com/in/desiree-pelay )
* **Ejes clave:** Diseño Organizacional, Estrategia HRBP, Neurociencias en Liderazgo, Upskilling para IA.
